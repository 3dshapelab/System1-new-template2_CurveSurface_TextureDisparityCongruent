...
// 
// 
// Method 1 draws separate triangles, which is closer to what the modern openGL does
// Method 2 uses glBegin(TRIANGLES...) and draw strips. It's more intuitive to people new to openGL.
...
// keypresses changes:
// m n to change stimulus depth
// + to switch between texture patterns
...
// Cylinder composed of strips (method 2)
...
void buildVertices_strips(double cylinderDepth);
void drawVertices_strips(int texNum);
int LoadGLTextures();
void drawInfo();
...
/*
			...
			texcoors[t_index] = u; //(x + stimulus_width / 2) / normalizer; //u coordinate.
			t_index++;
			texcoors[t_index] = v; //total_distance_y / normalizer; //v coordinate
			t_index++;
*/
...
void buildVertices_strips(double cylinderDepth) {
	...
	*/
	...
	double z_u_prev = calculateStimulus_ZfromY(cylinderDepth, -stimulus_height / 2); // set its initial value

	for (y_b = -stimulus_height / 2; y_b < stimulus_height / 2; y_b = y_b + step_size_height) {
		// y_bottom goes from -stimulus_height to the row below stimulus_height / 2
		...
		z_u = calculateStimulus_ZfromY(cylinderDepth, y_u);

		local_distance_v = sqrt(pow(y_b - y_u, 2) + pow(z_b - z_u, 2));

		for (double x = -stimulus_width / 2; x <= stimulus_width / 2; x = x + step_size_width) { //
			...
			v_index++;
			texcoors_byStrip[i_strip][t_index] = (x + stimulus_width / 2) / normalizer_u; //u coordinate
			t_index++;
			texcoors_byStrip[i_strip][t_index] = (total_distance_y + local_distance_v) / normalizer_v; //v coordinate
			...
			texcoors_byStrip[i_strip][t_index] = (x + stimulus_width / 2) / normalizer_u; //u coordinate
			t_index++;
			texcoors_byStrip[i_strip][t_index] = total_distance_y / normalizer_v; //v coordinate
			t_index++;
		}
		total_distance_y = total_distance_y + local_distance_v;
		i_strip++;
		z_u_prev = z_u;
	}
...
void drawVertices_strips(int texNum)
{
	...
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, loadedTexture[texNum]);               // Select Our Texture
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
*/
//string texturePath = "fall16-ailin-stimulusTest/test/gd.png";
	string texturePath = "C:/cncsvision/experimentsbrown/ShapeConstancy/textures/polkadots/0/polkadots1.png";
	loadedTexture[1] = SOIL_load_OGL_texture
	(
		texturePath.c_str(),
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
	...
	texturePath = "C:/cncsvision/experimentsbrown/ShapeConstancy/textures/polkadots/0/polkadots2.png";
	loadedTexture[2] = SOIL_load_OGL_texture
	(
		texturePath.c_str(),
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
	...
	texturePath = "C:/cncsvision/experimentsbrown/ShapeConstancy/textures/polkadots/0/polkadots3.png";
	loadedTexture[3] = SOIL_load_OGL_texture
	(
		texturePath.c_str(),
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
...
void handleKeypress(unsigned char key, int x, int y)
{
	...
	case '+':
	{
		//texnum_minus_one = (texnum_minus_one + 1) % 4;
		//texnum = texnum_minus_one + 1;
		depthID = (depthID + 1) % 3;
		texnum = depthID + 1;
